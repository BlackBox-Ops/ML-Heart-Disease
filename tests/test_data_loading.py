...
def test_raw_exits():
    assert os.path.exists("../data/processed/heart_disease_cleaned.csv"), "Datasheet tidak ditemukan"

def test_raw_data_schema():
    df = pd.read_csv("../data/processed/heart_disease_cleaned.csv")
    expected_cols = {"age", "sex", "cp", "trestbps", "chol", "fbs", "restecg", "thalch", "exang", "oldpeak", "slope", "ca", "thal", "dataset", "num"}
